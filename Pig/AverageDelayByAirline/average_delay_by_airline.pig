...
valid_flights = FILTER flights BY dep_delay IS NOT NULL;

grouped = GROUP valid_flights BY op_unique_carrier;

result = FOREACH grouped GENERATE
    group AS airline,
    AVG(valid_flights.dep_delay) AS average_departure_delay;

ordered = ORDER result BY average_departure_delay DESC;

STORE ordered INTO '/flight/pig_average_delay_by_airline'
    USING PigStorage('\t');