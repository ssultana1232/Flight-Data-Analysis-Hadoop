...
grouped = GROUP cancelled_flights BY op_unique_carrier;

result = FOREACH grouped GENERATE
    group AS airline,
    COUNT(cancelled_flights) AS cancelled_flights;

ordered = ORDER result BY cancelled_flights DESC;

STORE ordered INTO '/flight/pig_cancelled_flights_by_airline'
    USING PigStorage('\t');